...
// // unique keywords
// const keywords = {};
// Object.values(idToKeywords).forEach((val, i) => {
//   val.forEach((kWord) => keywords[kWord] = 1);
...
// const idToKeywords = {};
// data.forEach((d, i) => {
//   if (d.id) idToKeywords[d.id] = getKeywords(getSrchStr(d));
//   console.log(i + 1 + "/" + data.length);
// });
// let count = 0;
// Object.values(idToKeywords).forEach(val => count += val.length);
// console.log(count);
// console.log(Object.keys(idToKeywords).length);
...
//   });
//   IDF[kWord] = Math.log((Object.values(idToKeywords).length - count + 0.5)/(count + 0.5));
...
//     if (d.includes(kWord)) count++;
//   });
//   IDF[kWord] = Math.log(Object.values(idToKeywords).length / count);
//   console.log(i + 1 + "/" + keywords.length);
// });
...
//     });
//     const bm25 = {};
//     Object.keys(freq).forEach(kWord => bm25[kWord] = IDF[kWord] * ((freq[kWord] * (2 + 1)) / (freq[kWord] + 2*(1 - 0.75 + 0.75*(idToKeywords[id].length / L)))));
//     BM25[id] = bm25;
//     console.log(i+1 + "/" + Object.keys(idToKeywords).length);
// });
// let count = 0;
// Object.values(BM25).forEach(val => count += Object.keys(val).length);
// console.log(count);
// console.log(Object.keys(BM25).length)
...
//     (kWord) =>
//       (TF_IDF[id][kWord] =
//         (IDF[kWord] || 0) * (freq[kWord] / (idToKeywords[id].length || 1)))
//   );
//   console.log(i + 1 + "/" + Object.keys(idToKeywords).length);
// });

// fs.writeFile("cache/IDF2.json", JSON.stringify(IDF), (err) =>
