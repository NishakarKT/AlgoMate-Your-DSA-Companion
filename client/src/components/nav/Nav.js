...
import { ABOUT_ROUTE, DSA_ROUTE, HOME_ROUTE, NEWS_ROUTE, RESOURCES_ROUTE, SIGN_IN_ROUTE } from "../../constants/routes";
// contexts
        ...
        <div className="nav__container">
            ...
            <div className="nav" ref={navRef}>
                ...
                <div className="nav__right">
                    <NavLink exact="true" to={HOME_ROUTE} className={({ isActive }) => (isActive ? "nav__link nav__activeLink" : "nav__link")}><HomeRoundedIcon />Home</NavLink>
                    <NavLink exact="true" to={DSA_ROUTE} className={({ isActive }) => (isActive ? "nav__link nav__activeLink" : "nav__link")}><EqualizerRoundedIcon />DSA</NavLink>
                    <NavLink exact="true" to={NEWS_ROUTE} className={({ isActive }) => (isActive ? "nav__link nav__activeLink" : "nav__link")}><NewspaperRoundedIcon />News</NavLink>
                    <NavLink exact="true" to={RESOURCES_ROUTE} className={({ isActive }) => (isActive ? "nav__link nav__activeLink" : "nav__link")}><PsychologyRoundedIcon />Resources</NavLink>
                    ...
                    <Menu className="nav__basicMenu" anchorEl={avatarAnchor} open={Boolean(avatarAnchor)} onClose={() => setAvatarAnchor(null)}>
                        <div className="nav__menu">
                            ...
                            <div className="nav__menuLinks" onClick={() => setAvatarAnchor(null)}>
                                <div onClick={() => user.name ? handleSignOut() : navigate(SIGN_IN_ROUTE)} className="nav__link"><ExitToAppRoundedIcon />{user.name ? "Sign Out" : "Sign In"}</div>
                            </div>
                        </div>
                    </Menu>
                </div>
                ...
                <Menu className="nav__basicMenu nav__resMenu" anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={() => setMenuAnchor(null)}>
                    <div className="nav__menu">
                        <div className="nav__menuInfo">
                            <img className="nav__avatar" src={user.profilePic || avatarPic} alt="" onClick={() => { setMenuAnchor(null); navigate(HOME_ROUTE + "/" + user.email); }} />
                            <div className="nav__userInfo">
                                <p>{user.name || "Anonymous"}</p>
                                <p>{user.name ? "Welcome, " + user.name + "!" : "Why not sign in?"}</p>
                            </div>
